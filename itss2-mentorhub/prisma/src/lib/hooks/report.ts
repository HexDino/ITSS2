...
export function useCheckReport<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: string; reporterId?: string; targetType?: string; targetId?: string; reason?: string; status?: ReportStatus; resolution?: string }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
